...
CARD_TYPES: Dict[CardTypeEnum, List[pygame.Surface]] = {}
CARD_IMAGES: Dict[CardImageEnum, List[pygame.Surface]] = {}


def load_images() -> None:
    for t_val, (t_name, t_frame_cnt) in [(e, e.value) for e in CardTypeEnum]:
        CARD_TYPES.update(
            {
                t_val: [
                    pygame.image.load(
                        CARD_TYPE_ASSET_DIR / f"{t_name}" / f"card_{t_name}{i+1}.png"
                    ).convert_alpha()
                    for i in range(t_frame_cnt)
                ]
            }
        )
