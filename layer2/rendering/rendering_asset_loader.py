    ...
    for type, val in [(e, e.value) for e in enum]:
        surfs.update(
            {type: pygame.image.load(
                path / f"{name}{val}.png").convert_alpha()}
        )
...
def load_images() -> None:
    ...
    _load_image_type(MarkerEnum, CARD_MARKER_SURFS,
                     CARD_MARKER_ASSET_DIR, "marker")
    _load_image_type(TerrainEnum, TILE_TYPE_SURFS,
                     TILE_TYPE_ASSET_DIR, "tiles")
    _load_image_type(UnitTypeEnum, UNIT_TYPE_SURFS,
                     UNIT_TYPE_ASSET_DIR, "units")
    _load_image_type(
        SelectionTypeEnum, SELECTION_SURFS, SELECTION_ASSET_DIR, "tile_selections"
    )

    _load_animation_type(CardImageEnum, CARD_IMAGE_SURFS,
                         CARD_IMAGE_ASSET_DIR, "card_")
